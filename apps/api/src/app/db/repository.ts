...
export class Repository<T extends { _id: string }> {
  ...
  public async findOne(id: string): Promise<WithId<T> | null> {
    await this.initialised;
    return await this.db.findOne({ _id: id } as Filter<any>); /* eslint-disable-line @typescript-eslint/no-explicit-any */
  }
  ...
  public async count(query: Filter<T>): Promise<number> {
    await this.initialised;
    return await this.db.countDocuments(query as Filter<any>); /* eslint-disable-line @typescript-eslint/no-explicit-any */
  }

  public async update(item: T): Promise<T> {
    await this.initialised;
    await this.db.updateOne({ _id: item._id } as Filter<any>, item); /* eslint-disable-line @typescript-eslint/no-explicit-any */
    return item;
  }

  public async removeOne(query: Filter<T>): Promise<number> {
    await this.initialised;
    const result = await this.db.deleteOne(query as Filter<any>); /* eslint-disable-line @typescript-eslint/no-explicit-any */
