export interface TaxonomyEntry {
  ...
  label: string; // 'superDepartment';
  source: string; // 'essearch';
  pageType: string; // 'NONE';
}

declare const ProductRefSymbol: unique symbol;
declare const ProductIdSymbol: unique symbol;
declare const PromotionTypeRefSymbol: unique symbol;

export type ProductId = number & {
  [ProductIdSymbol]: never;
};

export type StringProductId = string & {
  [ProductIdSymbol]: never;
};

export type ProductRef = `ProductType:${ProductId}` & {
  [ProductRefSymbol]: never;
};

export type PromotionTypeRef = `PromotionType:${string}` & {
  [PromotionTypeRefSymbol]: never;
};

export interface CompositeResultType {
...
export interface PromotionType {
  ...
  startDate: string; // '2025-06-15T23:00:00Z';
  endDate: string; // '2025-07-01T23:00:00Z';
  description: string; // 'Â£1.70 Clubcard Price';
  unitSellingInfo: string; // 'Â£1.70\u002Flitre';
  price: PromotionPriceType;
  attributes: string[]; // ['CLUBCARD_PRICING']
}

export interface PromotionPriceType {
  __typename: 'PromotionPriceType';
  beforeDiscount: unknown; // null
  afterDiscount: number; // 2.1;
}
...
export interface ProductType {
  ...
  id: StringProductId;
  tpnb: string; // '73339773';
  tpnc: string; // '276761995';
  gtin: string; // '05411188112709';
  adId: unknown; // null
  baseProductId: string; // '73339773';
  title: string; // 'Alpro Almond No Sugar Long Life Dairy Free Drink 1L';
  brandName: string; // 'ALPRO';
  shortDescription: never; // null
  defaultImageUrl: string; // 'https:\u002F\u002Fdigitalcontent.api.tesco.com\u002Fv2\u002Fmedia\u002Fghs\u002Fc2197781-de6b-4601-9c12-4c0dd9cf9cbb\u002F7ef68106-f8b5-4b0b-8ace-cfb3998a2d9c.jpeg?h=225&w=225';
  superDepartmentId: string; // 'b;RnJlc2glMjBGb29k';
  media: {
  ...
  superDepartmentName: string; // 'Fresh Food';
  departmentId: string; // 'b;RnJlc2glMjBGb29kJTdDRGFpcnklMjBGcmVlJTIwJiUyMERhaXJ5JTIwQWx0ZXJuYXRpdmVz';
  departmentName: string; // 'Dairy Free & Dairy Alternatives';
  aisleId: string; // 'b;RnJlc2glMjBGb29kJTdDRGFpcnklMjBGcmVlJTIwJiUyMERhaXJ5JTIwQWx0ZXJuYXRpdmVzJTdDRGFpcnklMjBGcmVlJTIwRHJpbmtz';
  aisleName: string; // 'Dairy Free Drinks';
  shelfId: string; // 'b;RnJlc2glMjBGb29kJTdDRGFpcnklMjBGcmVlJTIwJiUyMERhaXJ5JTIwQWx0ZXJuYXRpdmVzJTdDRGFpcnklMjBGcmVlJTIwRHJpbmtzJTdDQWxtb25kJTIwRHJpbmtz';
  shelfName: string; // 'Almond Drinks';
  ...
  bulkBuyLimitGroupId: string; // 'G00000216';
  timeRestrictedDelivery: unknown; // null;
  restrictedDelivery: unknown; // null;
  isForSale: boolean; // true
  isInFavourites: unknown; // null
  isNew: boolean; // false
  isRestrictedOrderAmendment: unknown; // null
    ...
    components: [
      {
        ...
        isLowEverydayPricing: boolean; // false
      },
    ];
  };
  catchWeightList: unknown; // null
  price: {
    __typename: 'PriceType';
    actual: number; // 2.1;
    unitPrice: number; // 2.1;
    unitOfMeasure: string; // 'litre';
  };
  promotions: {
    __ref: PromotionTypeRef;
  }[];
  ...
  reviews: {
    ...
    stats: {
      ...
      noOfReviews: number; // 65;
      overallRating: number; // 4.3;
...
export interface ApolloCache {
  ROOT_QUERY: {
    ...
    'search(stringTODO)': {
      ...
      results: CompositeResultType[];
      facetGroups: unknown;
      popFilters: {
        __typename: 'ProductListFacetsType';
        category: string; // 'Pop Filter';
        categoryId: string; // 'popfilters';
        facets: {
      ...
      facets: {
        __ref: string; // 'FacetBooleanType:offer' | 'FacetMultiLevelType:category' | 'FacetListType:dietaries'
      }[];
      options: {
        __typename: 'ListOptionsType';
        sortBy: ['relevance', 'price-ascending', 'price-descending'];
      };
    };
  };
  'PromotionType:{"id":"93042295","description":"Â£1.70 Clubcard Price"}': PromotionType;
  'ProductType:276761995': ProductType;
  'FacetType:b;RnJlc2glMjBGb29kJTdDRGFpcnklMjBGcmVlJTIwJiUyMERhaXJ5JTIwQWx0ZXJuYXRpdmVzJTdDRGFpcnklMjBGcmVlJTIwRHJpbmtz': unknown;
  'FacetBooleanType:offer': unknown;
  'FacetMultiLevelType:category': unknown;
  'FacetListType:dietaries': unknown;
}

export interface TescoSearch {
  'mfe-header': unknown /*{
    ...
    props: {
      ...
      config: {
        client: {
          ...
          appBar: {
            menuItems: [
              ...
              },
              {
                ...
                filterIds: ['experienceLinks', 'customLinks'];
              },
              {
                id: 'app-bar-my-account';
                text: 'app-bar.signed-in.my-account';
                filterIds: ['authLinks', 'customLinks'];
              },
              {
                id: 'app-bar-sign-out';
                text: 'app-bar.signed-in.sign-out';
                filterIds: ['customLinks', 'authLinks'];
              },
            ];
          };
          appDownloadConfig: {
          ...
          cacheableExperimentKeys: ['GHS-UK_1972_searchBoxPosition'];
      ...
      config: {
        client: {
          ...
          dfp: {
            ...
            sizeMappings: [
              [[0, 531], ['fluid', [300, 250], [375, 258], [375, 110]]],
              [[532, 723], ['fluid', [532, 110], [300, 250]]],
              [[724, 917], 'fluid'],
              [[918, 50000], 'fluid'],
            ];
          };
