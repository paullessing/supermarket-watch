...
const waitroseUrls = {
  ...
  product: `https://www.waitrose.com/api/custsearch-prod/v3/search/${-1 /*customerId*/}/`,
};
...
async function streamFromUrl(url, resOrCurlArgs, maybeRes) {
  ...
  return await new Promise(async (resolve, reject) => {
    let bytes = 0;
    // console.log('curl', curlArgs); // debug, remove me
...
app.get(
  '/waitrose/custsearch-prod/v3/search/:customerId/:productId',
  async (req, res) => {
    try {
      ...
      const result = await streamFromUrl(
        `${waitroseUrls.product}/${encodeURIComponent(productId)}?${new URLSearchParams(req.query ?? {}).toString()}`,
        {
          headers: {
...
app.post(
  '/waitrose/content-prod/v2/cms/publish/productcontent/search/:customerId',
  express.json(),
  async (req, res) => {
    try {
      const customerId = parseInt(req.params.customerId, 10);
